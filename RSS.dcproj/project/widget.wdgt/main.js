...
function refreshFeed()
{
    if (!feed.url || feed.url.length < 1) {
        showMessageInContents(dashcode.getLocalizedString("Kein Feed angegeben"),
            dashcode.getLocalizedString("Gültige Feed-URL eingeben."));
        return false;
    }
...
function processFeedDocument(doc)
{
    ...
    if (doc) {
        ...
        if (results == null || results.length < 1) {
            showMessageInContents(dashcode.getLocalizedString("Keine Objekte gefunden"),
                dashcode.getLocalizedString("Der Feed enthält keine Einträge."));
            return;
        }
        ...
        if (results == null || results.length < 1) {
            showMessageInContents(dashcode.getLocalizedString("Keine anzuzeigenden Objekte"),
                dashcode.getLocalizedString("Der Feed enthält keine Einträge zu den angegebenen Kriterien."));
            return;
        }
        ...
        // document is empty
        showMessageInContents(dashcode.getLocalizedString("Feed ungültig"),
                              dashcode.getLocalizedString("%s ist anscheinend kein gültiger RSS- oder Atom-Feed.").replace("%s", feed.url));
    }
}
...
function showNewCount(newCount)
{
    setLoadingText(dashcode.getLocalizedString("%s neu").replace("%s", newCount));
}
...
function search(searchEvent)
{
    ...
    if (lastResults && lastResults.length) {
        ...
        if (searchResults == null || searchResults.length < 1) {
            showMessageInContents(dashcode.getLocalizedString("Keine Objekte gefunden"),
                dashcode.getLocalizedString("Keine Treffer zu diesen Suchbegriffen."));
        }
        else {
            // Generate the display
